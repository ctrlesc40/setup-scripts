...
#
# Note: On 19.04 hyper-v daemons are automatically installed but are 
#       disabled.
# The hyperv-daemons.hv-fcopy-daemon.service only starts if the "guest"
# features are enabled on the VM settings
#
systemctl enable hyperv-daemons.hv-fcopy-daemon.service
systemctl enable hyperv-daemons.hv-kvp-daemon.service
systemctl enable hyperv-daemons.hv-vss-daemon.service 

###############################################################################
# Configure Kernel Scheduller
#
sed -i_orig -e 's/GRUB_CMDLINE_LINUX_DEFAULT="quiet"/GRUB_CMDLINE_LINUX_DEFAULT="quiet elevator=noop"/g' /etc/default/grub
update-grub2
